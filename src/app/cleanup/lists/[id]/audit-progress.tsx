...
const STAGE_LABELS: Record<string, string> = {
  loading_lists: "Loading HubSpot lists…",
  scoring: "Scoring lists…",
...
export function AuditProgress({
  ...
  const pct =
    status.totalLists && status.totalLists > 0
      ? Math.min(100, Math.round((status.listsScanned / status.totalLists) * 100))
      : 0;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {status.totalLists
            ? `${status.listsScanned.toLocaleString()} of ${status.totalLists.toLocaleString()} lists`
            : "Enumerating lists…"}
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="text-muted-foreground mt-4 grid grid-cols-2 gap-4 text-sm">
          <div>
            <div className="text-xs uppercase tracking-wide">Lists scanned</div>
            <div className="text-2xl font-semibold tabular-nums">
              {status.listsScanned.toLocaleString()}
            </div>
          </div>
          <div>
        ...
        <p className="text-muted-foreground mt-6 text-xs">
          Usually finishes in under a minute. Progress is saved — safe to
          navigate away.
        </p>
      </CardContent>
    </Card>
