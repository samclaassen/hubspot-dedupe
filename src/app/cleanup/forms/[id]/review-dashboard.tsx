...
import { FindingCard, type ReviewFormFinding } from "./finding-card";
import { bulkArchiveFormsHighConfidence } from "./actions";
...
export function ReviewDashboard({
  ...
  const handleBulkArchive = () => {
    ...
    startTransition(async () => {
      const result = await bulkArchiveFormsHighConfidence(auditId, 0.95);
    ...
    <div>
      <section className="mb-6">
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          Scanned {totalForms.toLocaleString()} forms
        </p>
      </section>
      ...
      <section className="mb-6 flex flex-wrap items-center gap-2">
        {(Object.keys(FILTER_LABELS) as ReviewFilter[]).map((f) => {
          ...
          return (
            <Link
              key={f}
              href={`/cleanup/forms/${auditId}?filter=${f}&page=1`}
        ...
        {bulkEligible > 0 && (
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
                  Archive {bulkEligible} forms?
                </AlertDialogTitle>
                <AlertDialogDescription>
                  <strong>Reversible.</strong> Each form is re-fetched right
                  before archiving and re-scored. Only findings scoring ≥0.95
                  are processed.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      {pageCount > 1 && (
        <nav className="text-muted-foreground mt-6 flex items-center justify-between text-sm">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link
                href={`/cleanup/forms/${auditId}?filter=${filter}&page=${page - 1}`}
